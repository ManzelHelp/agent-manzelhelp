...
export default function CustomerNotificationsPage() {
  ...
  const getNotificationRedirectUrl = useCallback((notification: Notification): string | null => {
    ...
    if (notification.related_booking_id) {
      return `/customer/bookings/${notification.related_booking_id}`;
    }
    ...
    if (notification.related_job_id) {
      ...
      if (
        ...
        return `/customer/my-jobs/${notification.related_job_id}`;
      }
    ...
    if (notification.type === "message_received") {
      ...
      if (notification.related_job_id) {
        return `/customer/messages`; // Will need to find conversation by job_id
      }
    ...
    try {
      ...
      if (result.success) {
        ...
        toast({
          variant: "destructive",
          title: "Erreur",
          description: result.error || t("errors.markReadFailed"),
        });
      }
    } catch (err) {
      console.error("Unexpected error:", err);
    ...
    try {
      ...
      if (result.success) {
        ...
        toast({
          variant: "destructive",
          title: "Erreur",
          description: result.error || t("errors.markAllReadFailed"),
        });
      }
    } catch (err) {
      console.error("Unexpected error:", err);
    ...
    try {
      ...
      if (result.success) {
        ...
        toast({
          variant: "destructive",
          title: "Erreur",
          description: result.error || t("errors.bulkActionFailed"),
        });
      }
    } catch (err) {
      console.error("Unexpected error:", err);
      toast({
        variant: "destructive",
        title: "Erreur",
        description: t("errors.bulkActionFailed"),
      });
    }
  };
